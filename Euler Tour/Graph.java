...
class Graph implements Iterable<Vertex> {
	...
	public int numEdges;
	ArrayList<Vertex> oddDegVertices;
	...
	void addEdge(int a, int b, int weight) {
		...
		u.Adj.add(e);
		v.Adj.add(e);
		u.unvisitedEdges.add(e);
		v.unvisitedEdges.add(e);
		u.degree++;
		v.degree++;
		numEdges++;
	}
	...
	public static Graph readGraph(Scanner in, boolean directed) {
		...
		g.calcOddDegreeVertices();
		in.close();
		return g;
	}

	public void calcOddDegreeVertices() {
		for (Vertex v : this) {
			if (v.degree % 2 != 0)
				oddDegVertices.add(v);
		}
	}
}